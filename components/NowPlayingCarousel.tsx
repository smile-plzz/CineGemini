...
const NowPlayingCarousel: React.FC<NowPlayingCarouselProps> = ({ onSelectMovie }) => {
  ...
  if (loading) {
    return (
      <div className="w-full h-[400px] md:h-[500px] bg-white/5 rounded-3xl animate-pulse flex items-center justify-center">
        <div className="text-gray-500 uppercase font-black tracking-widest text-[10px]">Loading Featured Titles...</div>
      </div>
    );
  }
  ...
  return (
    <section className="relative overflow-hidden rounded-3xl">
      ...
      <div ref={scrollRef} className="flex gap-4 overflow-x-auto pb-6 snap-x no-scrollbar">
        {movies.map((movie) => (
          <div
            key={movie.id}
          ...
          >
            ...
            <div className="absolute bottom-0 inset-x-0 p-6 md:p-8 space-y-3 translate-y-2 group-hover/item:translate-y-0 transition-transform duration-500">
              <div className="flex items-center gap-3">
                <span className="px-3 py-1 bg-blue-600 rounded-full text-[9px] font-black uppercase tracking-widest text-white shadow-lg">Featured</span>
              ...
              <div className="flex items-center gap-4 opacity-0 group-hover/item:opacity-100 transition-opacity duration-500">
                <button
                  ...
                  className="flex items-center gap-2 bg-white text-black px-5 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-blue-600 hover:text-white transition-all"
                >
                  <Play size={12} fill="currentColor" /> Details
                </button>
                <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">
                  {movie.genre?.[0]} &bull; {movie.year}
                </span>
              </div>
            </div>
          </div>
